...
sed -i "s|\${SERVER_REDIRECT}|${SERVER_REDIRECT}|" /etc/nginx/conf.d/default.conf /etc/nginx/conf.d/default-ssl.conf
sed -i "s|\${SERVER_NAME}|${SERVER_NAME}|" /etc/nginx/conf.d/default.conf /etc/nginx/conf.d/default-ssl.conf
sed -i "s|\${SERVER_REDIRECT_CODE}|${SERVER_REDIRECT_CODE}|" /etc/nginx/conf.d/default.conf /etc/nginx/conf.d/default-ssl.conf
sed -i "s|\${SERVER_REDIRECT_POST_CODE}|${SERVER_REDIRECT_POST_CODE}|" /etc/nginx/conf.d/default.conf /etc/nginx/conf.d/default-ssl.conf
sed -i "s|\${SERVER_REDIRECT_PUT_PATCH_DELETE_CODE}|${SERVER_REDIRECT_PUT_PATCH_DELETE_CODE}|" /etc/nginx/conf.d/default.conf /etc/nginx/conf.d/default-ssl.conf
sed -i "s|\${SERVER_REDIRECT_PATH}|${SERVER_REDIRECT_PATH}|" /etc/nginx/conf.d/default.conf /etc/nginx/conf.d/default-ssl.conf
sed -i "s|\${SERVER_REDIRECT_SCHEME}|${SERVER_REDIRECT_SCHEME}|" /etc/nginx/conf.d/default.conf /etc/nginx/conf.d/default-ssl.conf

sed -i "s|\${SERVER_SSL_CERTIFICATE_FILE_NAME}|${SERVER_SSL_CERTIFICATE_FILE_NAME}|" /etc/nginx/includes/ssl-params.conf
sed -i "s|\${SERVER_SSL_CERTIFICATE_KEY_NAME}|${SERVER_SSL_CERTIFICATE_KEY_NAME}|" /etc/nginx/includes/ssl-params.conf

if [ "${SERVER_ENABLE_HTTP_TRAFFIC}" = "false" ]; then
    rm -f /etc/nginx/conf.d/default.conf
fi

if [ "${SERVER_ENABLE_HTTPS_TRAFFIC}" = "false" ]; then
